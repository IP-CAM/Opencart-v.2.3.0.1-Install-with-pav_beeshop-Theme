...
class ControllerSaleCustomer extends Controller {
	...
	protected function getList() {
		...
		if ($user['user_group_id'] == $this->config->get('config_user_group_id')) {
			$filter_data = array('filter_user_id' => $user['user_id']);
		}
		# Update End
		...
		foreach ($results as $result) {
			...
			if ($user) {
				$organaization = $user['firstname'] . ' ' . $user['lastname'];
			} else {
				$organaization = $this->language->get('text_not_supervised');
			}
			# Update End
		...
		# Update Start By: Ismail Ashour
		$data['column_supervisor'] = $this->language->get('column_supervisor');
		# Update End
	...
	protected function getForm() {
		...
		$data['entry_organaztion'] = $this->language->get('entry_organaztion');
		...
		$organaization = $this->model_user_user->getUser($user['user_id']);
		$data['organaization'] = $organaization['firstname'] . ' ' . $organaization['lastname'];

		$organaizations = $this->model_user_user->getUsers();

		$data['organaizations'] = array();

		foreach ($organaizations as $organaization) {
			if ($organaization['user_group_id'] == $this->config->get('config_user_group_id')) {
				$data['organaizations'][] = array(
					'user_id' => $organaization['user_id'],
					'name' => $organaization['firstname'] . ' ' . $organaization['lastname'],
				);
			}
		}
		...
		$data['config_customer_group_donor_id'] = $this->config->get('config_customer_group_donor_id');

		$data['products'] = array();

		if (array_key_exists('customer_id', $this->request->get)) {

			$results = $this->model_sale_customer->getProducts($this->request->get['customer_id']);
			...
			foreach ($results as $result) {
				...
				if ($product) {
					$data['products'][] = array(
						...
						'name' => $product['name'],
						'product_link' => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $product['product_id'], 'SSL'),
						'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
					);
				}
			}
		...
		if ($user['user_group_id'] == $this->config->get('config_user_group_id')) {
			foreach ($data['customer_groups'] as $key => $value) {
				if ($value['customer_group_id'] != $this->config->get('config_customer_group_benef_id')) {
		...
		if (isset($this->request->post['user_id'])) {
			...
			$data['user_id'] = $customer_info['user_id'];
		} else {
		...
		if (isset($this->request->post['featured'])) {
			...
			$data['featured'] = $customer_info['featured'];
		} else {
			$data['featured'] = 0;
		}
		# Update End
	...
	public function products() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')
			...
			if (!array_key_exists('product_id', $this->request->get)) {
				$this->model_sale_customer->deleteProductsByCustomerId($this->request->get['customer_id']);

				$post_products = explode(',', $this->request->post['products']);

				foreach ($post_products as $key => $product_id) {
					$this->model_sale_customer->addProduct($this->request->get['customer_id'], $product_id);
				}
			} else {
				if ($this->request->get['status'] != 'delete') {
					$this->model_sale_customer->updateCustomerProductStatus(
						$this->request->get['customer_id'],
						$this->request->get['product_id'],
						$this->request->get['status']);
				} else {
					$this->model_sale_customer->deleteProductsByCustomerId(
						$this->request->get['customer_id'],
						$this->request->get['product_id']);
				}
		...
		if (array_key_exists('customer_id', $this->request->get)) {
			$results = $this->model_sale_customer->getProducts($this->request->get['customer_id'], ($page - 1) * 10, 10);
			...
			foreach ($results as $result) {
				...
				if ($product) {
					$data['products'][] = array(
						...
						'status_label' => $this->language->get($result['product_status']),
						'activate_link' => $this->url->link('catalog/product/activate_product', 'token=' . $this->session->data['token'] . '&product_id=' . $product['product_id'] . '&customer_id=' . $result['customer_id'], 'SSL'),
						'delete_link' => $this->url->link('catalog/product/delete_product', 'token=' . $this->session->data['token'] . '&product_id=' . $product['product_id'] . '&customer_id=' . $result['customer_id'], 'SSL'),
						'product_link' => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $product['product_id'], 'SSL'),
						'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
					);
				}
		...
		$product_total = $this->model_sale_customer->getTotalProducts($this->request->get['customer_id']);

		$pagination = new Pagination();
		$pagination->total = $product_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/customer/product', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('sale/customer_product.tpl', $data));
	}
