...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		if ($user['user_group_id'] == $this->config->get('config_user_group_id')) {
			$filter_data['filter_user_id'] = $_SESSION['user_id'];
		}
		# Update End
	...
	public function info() {
		...
		if (isset($this->request->get['token'])) {
			$this->session->data['cookie'] = $this->request->get['token'];
		}
		...
		if ($order_info) {
			...
			# Update Start By: Ismail Ashour
			$data['tab_product_history'] = $this->language->get('tab_product_history');
			$data['text_product_history'] = $this->language->get('text_product_history');
			$data['entry_product_status'] = $this->language->get('entry_product_status');

			$data['column_recipient'] = $this->language->get('column_recipient');

			$data['entry_purchase_source'] = $this->language->get('entry_purchase_source');
			$data['entry_purchase_price'] = $this->language->get('entry_purchase_price');
			$data['entry_product_details'] = $this->language->get('entry_product_details');
			$data['entry_delivery_date'] = $this->language->get('entry_delivery_date');
			$data['entry_order_recipient'] = $this->language->get('entry_order_recipient');
			$config_complete_status = $this->config->get('config_complete_status');
			$data['config_complete_status'] = $config_complete_status[0];
			# Update End
			...
			foreach ($products as $product) {
				...
				# Update Start By: Ismail Ashour
				$this->load->model('sale/customer');
				$customer = $this->model_sale_customer->getCustomer($product['customer_id']);
				# Update End

				$data['products'][] = array(
					# Update Start By: Ismail Ashour
					'recipient_href' => ($customer) ? $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $customer['customer_id'], 'SSL') : '',
					'recipient_id' => ($customer) ? $customer['customer_id'] : '',
					'recipient' => ($customer) ? $customer['firstname'] . ' ' . $customer['lastname'] : '',
					# Update End
	...
	public function product_history() {
		...
		# Update Start By: Ismail Ashour
		$data['column_purchase_source'] = $this->language->get('column_purchase_source');
		$data['column_purchase_price'] = $this->language->get('column_purchase_price');
		$data['column_product_details'] = $this->language->get('column_product_details');
		$data['column_delivery_date'] = $this->language->get('column_delivery_date');
		$data['column_order_recipient'] = $this->language->get('column_order_recipient');
		...
		$results = $this->model_sale_order->getProductHistories(
		...
		foreach ($results as $result) {
			$data['histories'][] = array(
				...
				'purchase_source' => $result['purchase_source'],
				'purchase_price' => $result['purchase_price'],
				'product_details' => $result['product_details'],
				'delivery_date' => $result['delivery_date'],
				'order_recipient' => $result['order_recipient'],
				# Update End
				'comment' => nl2br($result['comment']),
				'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
			);
		...
		$history_total = $this->model_sale_order->getTotalProductHistories(
			$this->request->get['order_id']
			, $this->request->get['order_product_id']);
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/order/product_history', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&order_product_id=' . $this->request->get['order_product_id'] . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('sale/product_history.tpl', $data));
	}
	# Update End
