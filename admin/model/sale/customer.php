...
class ModelSaleCustomer extends Model {
	...
	public function getCustomers($data = array()) {
		...
		if (!empty($data['filter_product_status'])) {
			$sql .= " LEFT JOIN " . DB_PREFIX . "product_to_customer p2c ON(c.customer_id = p2c.customer_id) ";

			$implode[] = " p2c.status = '" . $this->db->escape($data['filter_product_status']) . "'";
		}
	...
	public function getTotalCustomers($data = array()) {
		...
		if (!empty($data['filter_product_status'])) {
			$sql .= " LEFT JOIN " . DB_PREFIX . "product_to_customer ON(" . DB_PREFIX . "customer.customer_id = " . DB_PREFIX . "product_to_customer.customer_id) ";

			$implode[] = " " . DB_PREFIX . "product_to_customer.status = '" . $this->db->escape($data['filter_product_status']) . "'";
		}
	...
	# Update Start By: Ismail Ashour
	public function deleteProductsByCustomerId($customer_id, $product_id = 0 ){
		
		if ($product_id == 0 ){
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_customer
			WHERE customer_id = '" . (int)$customer_id . "'");	
		} elseif($product_id != 0) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_customer
				WHERE customer_id = '" . (int)$customer_id . "' AND 
				product_id = '" . (int)$product_id . "';");	
		}
		
	}
	
	public function addProduct($customer_id, $product_id) {
	...
	public function updateCustomerProductStatus($customer_id, $product_id, $status) {
