        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-customer" class="form-horizontal">
          <ul class="nav nav-tabs">
            ...
            <?php if ($customer_id) { ?>
              ...
              <?php if($customer_group_id != $config_customer_group_donor_id): ?>
          ...
          <div class="tab-content">
                ...
                <div class="col-sm-10">
                  <div class="tab-content">
                    <div class="tab-pane active" id="tab-customer">
                      ...
                      <!-- # Update Start By: Ismail Ashour -->
                      <?php if ($user_group=='org'): ?>
                        <div class="form-group">
                        <label class="col-sm-2 control-label" for="input-user-group">
                          <?php echo $entry_organaztion; ?>
                        </label>
                        <div class="col-sm-10">
                          <input type="text" class="form-control" value="<?php echo $organaization; ?>" disabled="disabled" />
                        </div>
                      </div>
                      <input type="hidden" name="user_id" value="<?php echo $user_id; ?>" />
                      <?php else: ?>
                      <div class="form-group">
                        <label class="col-sm-2 control-label" for="input-user-group"><?php echo $entry_organaztion; ?></label>
                        <div class="col-sm-10">
                          <select name="user_id" id="input-user" class="form-control">
                          <option value="" selected="selected"><?php echo $text_select ?></option>
                            <?php foreach ($organaizations as $organaization) { ?>
                            <?php if ($organaization['user_id'] == $user_id) { ?>
                            <option value="<?php echo $organaization['user_id']; ?>" selected="selected"><?php echo $organaization['name']; ?></option>
                            <?php } else { ?>
                            <option value="<?php echo $organaization['user_id']; ?>"><?php echo $organaization['name']; ?></option>
                            <?php } ?>
                            <?php } ?>
                          </select>
                        </div>
            ...
            <div class="tab-pane" id="tab-products">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <input type="text" name="customer_products" value="" placeholder="<?php echo $entry_product; ?>" id="input-customer-products" class="form-control" />
                  <div id="customer-product" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($products as $customer_product) { ?>
                    <div id="customer-product<?php echo $customer_product['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $customer_product['name']; ?>
                      <input type="hidden" name="customer_product[]" value="<?php echo $customer_product['product_id']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
                </div>
              ...
              <div class="text-right">
                <button id="button-products" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?php echo $button_product_add; ?></button>
              </div>
            </div>
            <?php endif; ?>
            <!-- # Update End -->
...
// Related
$('#products').load('index.php?route=sale/customer/products&token=<?php echo $token; ?>&customer_id=<?php echo $customer_id; ?>');

// Related
$('input[name=\'customer_products\']').autocomplete({
  ...
  'select': function(item) {
    $('input[name=\'customer_products\']').val('');

    $('#customer-product' + item['value']).remove();

    $('#customer-product').append('<div id="customer-product' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="customer_product[]" value="' + item['value'] + '" /></div>');  
  } 
});
...
$('#button-products').on('click', function(e) {
  e.preventDefault();
  
  var values = $("input[name='customer_product[]']")
              .map(function(){return $(this).val();}).get();

  $.ajax({
    url: 'index.php?route=sale/customer/products&token=<?php echo $token; ?>&customer_id=<?php echo $customer_id; ?>',
    type: 'post',
    dataType: 'html',
    data: 'products=' + encodeURIComponent(values),
    beforeSend: function() {
      $('#button-products').button('loading');
    },
    complete: function() {
      $('#button-products').button('reset');
    },
