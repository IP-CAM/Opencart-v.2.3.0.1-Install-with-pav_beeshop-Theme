      ...
      <div class="panel-body">
        ...
        <div class="tab-content">
          ...
          <fieldset>
            ...
            <div id="history-<?php echo $product['order_product_id']; ?>"></div>
            <br />
              <form class="form-horizontal" id="history-form-<?php echo $product['order_product_id']; ?>">
                <!-- Start Update By: Ismail Ashour -->
                <div class="form-group organaztion_notes">
                ...
                <div class="form-group organaztion_notes">
                  <label class="col-sm-2 control-label">
                    <?php echo $entry_delivery_date; ?>
                  </label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="delivery_date" value="" />
                  </div>
                </div>
                <div class="form-group organaztion_notes">
                  <label class="col-sm-2 control-label">
              ...
              <div class="text-right">
                <button id="button-history-<?php echo $product["order_product_id"] ?>" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?php echo $button_history_add; ?></button>
              </div>
            <br />
          <?php endforeach; ?>
          </fieldset>
...
$('#history-<?php echo $product["order_product_id"] ?>').delegate('.pagination a', 'click', function(e) {

	e.preventDefault();
	
	$('#history-<?php echo $product["order_product_id"] ?>').load(this.href);
});

$('#history-<?php echo $product["order_product_id"] ?>').load('index.php?route=sale/order/history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&order_product_id=<?php echo $product["order_product_id"] ?>');

$('#button-history-<?php echo $product["order_product_id"] ?>').on('click', function() {
  ...
  $.ajax({
    url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order/history&order_id=<?php echo $order_id; ?>&order_product_id=<?php echo $product["order_product_id"] ?>',
    type: 'post',
    dataType: 'json',
    data: 'order_status_id=' + encodeURIComponent($('#history-form-<?php echo $product["order_product_id"] ?> select[name=\'order_status_id\']').val()) + '&notify=' + ($('#history-form-<?php echo $product["order_product_id"] ?> input[name=\'notify\']').prop('checked') ? 1 : 0) + '&append=' + ($('#history-form-<?php echo $product["order_product_id"] ?> input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('#history-form-<?php echo $product["order_product_id"] ?> textarea[name=\'comment\']').val()) + '&purchase_source=' + encodeURIComponent($('#history-form-<?php echo $product["order_product_id"] ?> input[name=\'purchase_source\']').val()) + '&purchase_price=' + encodeURIComponent($('#history-form-<?php echo $product["order_product_id"] ?> input[name=\'purchase_price\']').val()) + '&product_details=' + encodeURIComponent($('#history-form-<?php echo $product["order_product_id"] ?> input[name=\'product_details\']').val())  + '&delivery_date=' + encodeURIComponent($('#history-form-<?php echo $product["order_product_id"] ?> input[name=\'delivery_date\']').val())  + '&order_recipient=' + encodeURIComponent($('#history-form-<?php echo $product["order_product_id"] ?> input[name=\'order_recipient\']').val()) + '&order_product_id=<?php echo $product["order_product_id"] ?>',
    beforeSend: function() {
      $('#button-history-<?php echo $product["order_product_id"] ?>').button('loading');      
    },
    complete: function() {
      $('#button-history-<?php echo $product["order_product_id"] ?>').button('reset');  
    },
    success: function(json) {
      $('.alert').remove();

      if (json['error']) {
        $('#history-<?php echo $product["order_product_id"] ?>').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
      } 

      if (json['success']) {
        $('#history-<?php echo $product["order_product_id"] ?>').load('index.php?route=sale/order/history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&order_product_id=<?php echo $product["order_product_id"]; ?>');
        
        $('#history-<?php echo $product["order_product_id"] ?>').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
...
function changeStatus(){
  ...
  $.ajax({
    ...
    success: function(html) {
      $('#history-<?php echo $product["order_product_id"] ?>').after(html);
    }
  });
}
...
$('#history-form-<?php echo $product["order_product_id"] ?> select[name="order_status_id"]').change(function(){ 
  
  if ($( this ).val() == <?php echo $config_complete_status ?> ){
    $('#history-form-<?php echo $product["order_product_id"] ?> .organaztion_notes').css('display', 'block');
  } else {
    $('#history-form-<?php echo $product["order_product_id"] ?> .organaztion_notes').css('display', 'none');
  }
