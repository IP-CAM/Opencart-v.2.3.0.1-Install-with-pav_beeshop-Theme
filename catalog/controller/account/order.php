	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				# Update Start By: Ismail Ashour
				$customer = $this->model_account_customer->getCustomer($product['customer_id']);
			...
			foreach ($products as $product) {
				...
				$results = $this->model_account_order->getOrderHistories($this->request->get['order_id'], $product['order_product_id']);

				foreach ($results as $result) {

					$data['histories'][$product['product_id']][] = array(

						'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
						'status'     => $result['status'],
						'comment'    => $result['notify'] ? nl2br($result['comment']) : ''
					);
					$data['histories'][$product['product_id']]['name'] = $result['product_name'];
				}
			}
			*/
			# Update End
