...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['text_beneficiaries'] = $this->language->get('text_beneficiaries');
			$data['text_no_beneficiaries'] = $this->language->get('text_no_beneficiaries');
			...
			$customers = $this->model_catalog_product->getCustomersByProductId($product_id);

			$data['customers'] = array();

			foreach ($customers as $customer){

				$custom_fields_array = unserialize($customer['custom_field']);

				$personal_pic_code = $custom_fields_array[6];

				$personal_pic_array = $this->model_tool_upload->getUploadByCode($personal_pic_code);

				$personal_pic = $personal_pic_array['filename'];
				
				$customer_pic = $this->model_tool_image->resize($personal_pic, 150, 200);

				$data['customers'][] = array(
					'customer_id' => $customer['customer_id'],
					'thumb' => $customer_pic,
					'name' => $customer['firstname'] . ' ' . $customer['lastname'],
					'href' => $this->url->link('account/profile', 'customer_id=' . $customer['customer_id']),
				);
			}
			# Update End
