...
class ModelMmosSocialAuth extends Model {

    public function findCustomerByIdentifier($provider, $identifier) {
        $result = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_authentication WHERE provider = '" . $this->db->escape($provider) . "' AND identifier = MD5('" . $this->db->escape($identifier) . "') LIMIT 1");
    ...
    public function findCountry($country) {
        $result = $this->db->query("SELECT country_id FROM " . DB_PREFIX . "country WHERE name = '" . $this->db->escape($country) . "' OR iso_code_2 = '" . $this->db->escape($country) . "' OR iso_code_3 = '" . $this->db->escape($country) . "' LIMIT 1");
    ...
    public function findZone($zone) {
        $result = $this->db->query("SELECT zone_id FROM " . DB_PREFIX . "zone WHERE name = '" . $this->db->escape($zone) . "' LIMIT 1");
    ...
    public function getCountryByZone($zone_id){
        $result = $this->db->query("SELECT country_id FROM " . DB_PREFIX . "zone WHERE zone_id = '" . (int) $zone_id . "' LIMIT 1");
    ...
    public function addCustomer($data) {
		...
		if (version_compare(VERSION, '2.0.1.1') == 0) {
        ...
        $mail->hostname = $this->config->get('config_mail_smtp_host');
        ...
        if ($data['email'] && preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $data['email'])) {
