...
class ModelCheckoutOrder extends Model {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product
					...
					name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int) $product['quantity'] . "', price = '" . (float) $product['price'] . "', total = '" . (float) $product['total'] . "', tax = '" . (float) $product['tax'] . "', reward = '" . (int) $product['reward'] . "'");

				$order_product_id = $this->db->getLastId();

				$this->db->query("UPDATE  " . DB_PREFIX . "product_to_customer
					SET status = 'waiting'
					WHERE customer_id = '" . (int) $product['customer_id'] . "' AND
					product_id = '" . (int) $product['product_id'] . "'");
	...
	# Update Start By: Ismail Ashour
	public function addOrderProductHistory(
		$order_id,
		$order_product_id,
		$order_status_id,
		$comment = '',
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product_history SET
			order_id = '" . (int) $order_id . "',
			order_product_id = '" . (int) $order_product_id . "',
			purchase_source = '" . $this->db->escape($purchase_source) . "',
			purchase_price = '" . $this->db->escape($purchase_price) . "',
			product_details = '" . $this->db->escape($product_details) . "',
			delivery_date = '" . $this->db->escape($delivery_date) . "',
			order_recipient = '" . $this->db->escape($order_recipient) . "',
			order_status_id = '" . (int) $order_status_id . "',
			notify = '" . (int) $notify . "',
			comment = '" . $this->db->escape($comment) . "',
			date_added = NOW()");

		$result = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int) $recipient_id . "';");

		$benificiary = $result->row;
		...
		if (in_array($order_status_id, $this->config->get('config_complete_status'))) {
			...
			foreach ($order_product_query->rows as $product) {
				...
				$this->db->query("UPDATE " . DB_PREFIX . "product_to_customer
					...
					AND customer_id = '" . (int) $benificiary['customer_id'] . "';");
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if ($this->config->get('config_order_mail')) {
					...
					# Update Start By: Ismail Ashour
					$customer_organaiztions = $this->db->query("
						SELECT *,
						uu.email AS email
						 FROM " . DB_PREFIX . "order_product op
						LEFT JOIN " . DB_PREFIX . "customer cc ON (op.customer_id = cc.customer_id)
						LEFT JOIN " . DB_PREFIX . "user uu ON (cc.user_id = uu.user_id)
						WHERE order_id = '" . (int) $order_id . "';");

					$org_emails = array();

					foreach ($customer_organaiztions->rows as $org) {
					...
					# Update Start By: Ismail Ashour
					foreach ($org_emails as $org_email) {

						$mail->setTo($org_email);
						$mail->send();
					}
					# Update End
