...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_to_customer p2cc
		ON (p2cc.product_id = p.product_id) ";

		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p2cc.status = 'active'";
	...
	public function getProductsByCustomerId($customer_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p
			...
			LEFT JOIN " . DB_PREFIX . "product_to_customer p2cc ON (p.product_id = p2cc.product_id)
			WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'
			AND p2cc.customer_id = '" . (int)$customer_id . "' 
			AND p2cc.status = 'active'
	...
	public function getCustomersByProductId($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer c
			LEFT JOIN " . DB_PREFIX . "product_to_customer p2c ON (c.customer_id = p2c.customer_id)
			LEFT JOIN " . DB_PREFIX . "product p ON (p2c.product_id = p.product_id)
			LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)
			WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'
			AND p.product_id = '" . (int)$product_id . "' ORDER BY pd.name DESC");
